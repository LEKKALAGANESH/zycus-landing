(() => {
  ...
  const announce = (msg) => { liveRegion.textContent = ''; setTimeout(() => (liveRegion.textContent = msg), 60); };

  const showStep = (idx) => {
    steps.forEach((step, i) => { step.hidden = i !== idx; });
  };
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        window.dataLayer.push({ event: 'generate_lead', form_id: 'zycus_demo' });
      ...
      if (data.errorType === 'database') {
        ...
        announce('Connection interrupted. Your request could not be processed.');
        restoreButton();
        return;
      }
  ...
  function showApologyModal(headline, body) {
    ...
    modal.addEventListener('click', (ev) => {
      if ((ev.target instanceof HTMLElement) && ev.target.hasAttribute('data-close')) close();
    });
